...
- Build docs: `uv run sphinx-build docs/source docs/_build`. `docs/source/api` (autosummary stubs) and `docs/source/examples` (sphinx-gallery output) are build artifacts (gitignored except for a `README.md` in each) that are only regenerated for files that changed, so a stale stub/gallery page left over from renaming or moving a module or example script can reference something that no longer exists and break the build with an `ImportExceptionGroup`. If the build fails after renaming/moving modules or examples, delete both directories and restore their READMEs first: `rm -rf docs/_build docs/source/api docs/source/examples && git checkout -- docs/source/api/README.md docs/source/examples/README.md` (PowerShell: `Remove-Item -Recurse -Force docs/_build, docs/source/api, docs/source/examples; git checkout -- docs/source/api/README.md, docs/source/examples/README.md`), then rebuild.
