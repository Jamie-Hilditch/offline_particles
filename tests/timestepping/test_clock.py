...
class TestClockConstructionValidation:
    def test_rejects_2d_time_array(self, make_clock) -> None:
        time_array = np.ones((3, 3), dtype=np.float64)
        with pytest.raises(ValueError, match="1D"):
            make_clock(time_array, 1.0)

    def test_rejects_single_element_time_array(self, make_clock) -> None:
        time_array = np.array([0.0], dtype=np.float64)
        with pytest.raises(ValueError, match="at least 2"):
            make_clock(time_array, 1.0)

    def test_rejects_non_increasing_time_array(self, make_clock) -> None:
        time_array = np.array([0.0, 2.0, 1.0], dtype=np.float64)
        with pytest.raises(ValueError, match="strictly increasing"):
            make_clock(time_array, 1.0)

    def test_rejects_constant_time_array(self, make_clock) -> None:
    ...
    def test_accepts_two_element_time_array(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0], dtype=np.float64)
        clock = make_clock(time_array, np.float64(0.5))
...
class TestClockProperties:
    def test_dt_property(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
    ...
    def test_forward_in_time_for_positive_dt(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 1.0)
        assert clock.forward_in_time

    def test_forward_in_time_for_negative_dt(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, -1.0)
        assert not clock.forward_in_time

    def test_first_time_forward(self, make_clock) -> None:
        time_array = np.array([1.0, 2.0, 3.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        assert clock.first_time == pytest.approx(1.0)

    def test_first_time_backward(self, make_clock) -> None:
        time_array = np.array([1.0, 2.0, 3.0], dtype=np.float64)
        clock = make_clock(time_array, -0.5)
        assert clock.first_time == pytest.approx(3.0)

    def test_final_time_forward(self, make_clock) -> None:
        time_array = np.array([1.0, 2.0, 3.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        assert clock.final_time == pytest.approx(3.0)

    def test_final_time_backward(self, make_clock) -> None:
        time_array = np.array([1.0, 2.0, 3.0], dtype=np.float64)
        clock = make_clock(time_array, -0.5)
        assert clock.final_time == pytest.approx(1.0)

    def test_time_array_property(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        np.testing.assert_array_equal(clock.time_array, time_array)

    def test_iteration_property_initial(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        assert clock.iteration == 0

    def test_tinfo_property(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        tinfo = clock.tinfo
...
class TestClockSetDt:
    def test_set_dt_forward(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        clock.set_dt(np.float64(0.25))
        assert clock.dt == pytest.approx(0.25)

    def test_set_dt_rejects_negative_for_forward(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        with pytest.raises(ValueError, match="positive"):
            clock.set_dt(np.float64(-0.5))

    def test_set_dt_rejects_positive_for_backward(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, -0.5)
        with pytest.raises(ValueError, match="negative"):
            clock.set_dt(np.float64(0.5))

    def test_set_dt_rejects_zero_for_forward(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        with pytest.raises(ValueError, match="positive"):
...
class TestClockSetTime:
    def test_set_time_valid(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        clock.set_time(np.float64(1.5))
        assert clock.time == pytest.approx(1.5)

    def test_set_time_out_of_bounds_raises(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        with pytest.raises(ValueError, match="out of bounds"):
            clock.set_time(np.float64(5.0))

    def test_set_time_updates_tidx(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
...
class TestClockSetIteration:
    def test_set_iteration_valid(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        clock.set_iteration(5)
        assert clock.iteration == 5

    def test_set_iteration_negative_raises(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
...
class TestClockAdvanceTime:
    def test_advance_time_forward(self, make_clock) -> None:
    ...
    def test_advance_time_backward(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0, 3.0], dtype=np.float64)
        clock = make_clock(time_array, -1.0)
        clock.advance_time()
    ...
    def test_advance_time_updates_tidx(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0, 3.0], dtype=np.float64)
        clock = make_clock(time_array, 1.0)
        clock.advance_time()
        assert clock.tidx == pytest.approx(1.0)

    def test_advance_time_increments_iteration(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        for _ in range(3):
            clock.advance_time()
...
class TestClockGetTimeIndex:
    def test_midpoint_interpolation(self, make_clock) -> None:
        time_array = np.array([0.0, 2.0, 4.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        idx = clock.get_time_index(np.float64(1.0))
        assert idx == pytest.approx(0.5)

    def test_out_of_bounds_raises(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        with pytest.raises(ValueError, match="out of bounds"):
            clock.get_time_index(np.float64(-0.1))

    def test_out_of_bounds_high_raises(self, make_clock) -> None:
        time_array = np.array([0.0, 1.0, 2.0], dtype=np.float64)
        clock = make_clock(time_array, 0.5)
        with pytest.raises(ValueError, match="out of bounds"):
