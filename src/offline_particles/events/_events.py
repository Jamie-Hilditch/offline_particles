...
class Event:
    ...
    def __init__(self, name: str, func: EventFunction, **kernels: Iterable[BoundKernel]) -> None:
        """Initialize the event."""
        self._name = name
        self._func = func
        self._kernels: dict[str, tuple[BoundKernel]] = {name: tuple(kernels) for name, kernels in kernels.items()}
    ...
    @property
    def kernels(self) -> Mapping[str, tuple[BoundKernel, ...]]:
