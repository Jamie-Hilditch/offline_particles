"""Kernels implementing relaxation forcing with a scalar relaxation coefficient."""
...
def construct_relaxation_kernel_scalar_coefficient_constant_target(
    prop: str,
    tendency: str,
    ...
    """Construct a kernel to apply relaxation forcing to a particle property with scalar coefficient and constant target.
    ...
    -------
    BoundKernel
        The constructed kernel for applying relaxation forcing to a particle property with scalar coefficient and constant target.
    ...
    if form == "linear":
        kernel_fn = _linear_relaxation_scalar_coefficient_constant_target(dtype(target))
    elif form == "quadratic":
        kernel_fn = _quadratic_relaxation_scalar_coefficient_constant_target(dtype(target))
    else:
    ...
    kernel = ParticleKernel(
        ...
        scalars=[
            ScalarDeclaration("relaxation_coefficient", dtype),
        ],
    )
...
def construct_relaxation_kernel_scalar_coefficient_property_target(
    prop: str,
    tendency: str,
    relaxation_coefficient: str,
    target: str,
    ...
    """Construct a kernel to apply relaxation forcing to a particle property with scalar coefficient and property target.
    ...
    -------
    BoundKernel
        The constructed kernel for applying relaxation forcing to a particle property with scalar coefficient and property target.
    ...
    if form == "linear":
        kernel_fn = _linear_relaxation_scalar_coefficient_property_target()
    elif form == "quadratic":
        kernel_fn = _quadratic_relaxation_scalar_coefficient_property_target()
    else:
        raise ValueError(f"Invalid form: {form}. Must be 'linear' or 'quadratic'.")
...
def construct_relaxation_kernel_scalar_coefficient_scalar_target(
    prop: str,
    tendency: str,
    relaxation_coefficient: str,
    target: str,
    ...
    """Construct a kernel to apply relaxation forcing to a particle property with scalar coefficient and scalar target.
    ...
    -------
    BoundKernel
        The constructed kernel for applying relaxation forcing to a particle property with scalar coefficient and scalar target.
    ...
    if form == "linear":
        kernel_fn = _linear_relaxation_scalar_coefficient_scalar_target()
    elif form == "quadratic":
        kernel_fn = _quadratic_relaxation_scalar_coefficient_scalar_target()
    else:
        raise ValueError(f"Invalid form: {form}. Must be 'linear' or 'quadratic'.")
...
def construct_relaxation_kernel_scalar_coefficient_field_target(
    prop: str,
    tendency: str,
    relaxation_coefficient: str,
    target: str,
    ...
    """Construct a kernel to apply relaxation forcing to a particle property with scalar coefficient and field target.
    ...
    -------
    BoundKernel
        The constructed kernel for applying relaxation forcing to a particle property with scalar coefficient and field target.
    ...
    match form, array_layout.axes:
        # 1D field target
        case "linear", (_,):
            kernel_function_impl = _linear_relaxation_scalar_coefficient_1D_field_target_impl(interpolation_half_width)
        case "quadratic", (_,):
            kernel_function_impl = _quadratic_relaxation_scalar_coefficient_1D_field_target_impl(
                interpolation_half_width
            )
        # 2D field target
        case "linear", (_, _):
            kernel_function_impl = _linear_relaxation_scalar_coefficient_2D_field_target_impl(interpolation_half_width)
        case "quadratic", (_, _):
            kernel_function_impl = _quadratic_relaxation_scalar_coefficient_2D_field_target_impl(
                interpolation_half_width
            )
        # 3D field target
        case "linear", (_, _, _):
            kernel_function_impl = _linear_relaxation_scalar_coefficient_3D_field_target_impl(interpolation_half_width)
        case "quadratic", (_, _, _):
            kernel_function_impl = _quadratic_relaxation_scalar_coefficient_3D_field_target_impl(
                interpolation_half_width
            )
        case _:
