...
from ..._kernels import kernel_function
...
def compute_z_kernel_function(
    ...
    if h_max_idx_0 < 0 or h_max_idx_1 < 0:
        raise ValueError("h field must have at least 2 points in each dimension to avoid out-of-bounds memory access.")
    ...
    for i in numba.prange(status.shape[0]):  # ty: ignore[not-iterable]
        ...
        h_value = bilinear_interpolation_particle(h_array, yidx[i] + h_offy, xidx[i] + h_offx, h_max_idx_0, h_max_idx_1)
