...
cdef inline double _sigma_coordinate(double zidx, int Nz) nogil:
    """Compute the sigma coordinate from a vertical index."""
    return (zidx + 0.5) / Nz - 1.0

cdef inline double _S_coordinate(double hc, double sigma, double h, double C) nogil:
    """Compute the S-coordinate transformation for ROMS vertical coordinates."""
    return (hc * sigma + h * C) / (hc + h)

cdef inline double _z_coordinate(real S, real h, real zeta) nogil:
    """Convert S-coordinate to physical coordinate."""
    return zeta + (zeta + h) * S

cdef inline double compute_z(double zidx, int Nz, double hc, double h, double C, double zeta) nogil:
...
cdef inline double _S_from_z(real z, real h, real zeta) nogil:
    """Convert physical coordinate to S-coordinate."""
    return (z - zeta) / (zeta + h)

cdef inline double _sigma_from_Cidx(int Cidx, double C_offset, int Nz) nogil:
...
cdef inline int _compute_Cidx_from_S(double S, double hc, int NZ, double h, double zeta, double[::1] readonly C, double C_offset) nogil:
...
cdef inline double _zidx_from_S(double S, double hc, int NZ, double h, double zeta, double[::1] readonly C, double C_offset) nogil:
    """Compute the vertical index from the S coordinate."""
    # Find the C index corresponding to S
    cdef int Cidx = _compute_Cidx_from_S(S, hc, NZ, h, zeta, C, C_offset)    
    # Linear interpolation to find the fractional index
    cdef double S_low = _S_coordinate(hc, sigma_from_Cidx(Cidx, C_offset, NZ), h, C[Cidx])
    cdef double S_high = _S_coordinate(hc, sigma_from_Cidx(Cidx + 1, C_offset, NZ), h, C[Cidx + 1])
    cdef double f = (S - S_low) / (S_high - S_low)
    return Cidx - C_offset + f

cdef inline double compute_zidx(double z, double h, double zeta, double hc, int NZ, double[::1] readonly C, double C_offset) nogil:
